import jwt
from models import Comment, HasComments, Post
...
def query_comment(
    ...
    relations = session.exec(select(HasComments).where(HasComments.post_id==post_id)).all()
...
def add_comment(post_id: int = Body(..., embed=True), comment: Comment = Body(..., embed=True), token: str = Depends(oauth2_scheme)):
    ...
    session.add(HasComments(comment_id=comment.comment_id, post_id=post.post_id))
...
def delete_comment(comment_id: int = Body(..., embed=True)):
    ...
    has_comments = session.exec(select(HasComments).where(HasComments.comment_id==comment_id)).all()
